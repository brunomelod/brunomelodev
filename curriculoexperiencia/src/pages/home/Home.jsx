          ...
          <div className="about-text">
            {selectedAboutTab === 'resumo' ? (
              <>
                ...
                <p>
                  {language === 'pt-BR' ? 'No backend, trabalho com frameworks modernos como' : 'On the backend, I work with modern frameworks like'} <strong>{t('about.highlights.springBoot')}</strong> {language === 'pt-BR' ? '(Java),' : '(Java),'} <strong>{t('about.highlights.fastapi')}</strong> {language === 'pt-BR' ? '(Python),' : '(Python),'} <strong>NodeJS</strong>{language === 'pt-BR' ? '(Express)' : '(Express)'} {language === 'pt-BR' ? 'desenvolvendo soluções escaláveis e de alta performance. No frontend, utilizo' : 'developing scalable and high-performance solutions. On the frontend, I use'} <strong>{t('about.highlights.reactjs')}</strong> {language === 'pt-BR' ? 'com' : 'with'} <strong>{t('about.highlights.tailwind')}</strong> {language === 'pt-BR' ? 'e' : 'and'} <strong>{t('about.highlights.styledComponents')}</strong>, {language === 'pt-BR' ? 'além de' : 'in addition to'} <strong>{t('about.highlights.html')}</strong> {language === 'pt-BR' ? 'e' : 'and'} <strong>{t('about.highlights.css')}</strong> {language === 'pt-BR' ? 'para criar interfaces modernas e responsivas.' : 'to create modern and responsive interfaces.'}
                </p>
                <p>
                  {t('about.resumo.p4')}
                </p>
